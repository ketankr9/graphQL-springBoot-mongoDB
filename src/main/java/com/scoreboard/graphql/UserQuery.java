...
public class UserQuery implements GraphQLQueryResolver {
  ...
  // The function name should be exactly same as that in the graphqlSchema
  // defined, "getAllUsers"
  ...
  public List<User> getTopNUsers(final Integer count) {
    ...
    // make sure to define findByScoreIsGreaterThanOrderByScoreDesc in the
    // UserRepository.java interface
    Stream<User> str =
        userRepository.findByScoreIsGreaterThanOrderByScoreDesc(0);

    // if no argument provided, return all users (score > 0) sorted in
    // descending order by user.score
    if (count == null)
